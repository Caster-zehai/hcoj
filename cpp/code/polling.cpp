...
int main(int argc, char* argv[]){
	...
	const int QUEUEING = 10;// queueing status code
	...
	try {
		...
		while (true) {
			...
			const char* sql = "select run_id,cid,pno,language,code from status where result = ? order by run_id";
			...
			while (rs->next()){
				...
				/*Change status after joining the queue*/
				const char* sql1 ="update status set result=? where run_id=?";
				ps = conn->prepareStatement(sql1);
				ps->setInt(1,11);ps->setString(2,runId);
				ps->executeUpdate();
				Submission submission(runId, cid, pno, lang, code);
				submissionQueue.push(submission);
				std::cout<<"add one:"<<submission.runId<<"\n";
			}
