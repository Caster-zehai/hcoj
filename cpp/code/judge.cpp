...
void updateSubmitStatus(const char* runId, int result, const char* ceInfo) {
	...
	try {
		...
		const char* sql = "update status set result=? where run_id=?";
...
void updateSubmitStatus(const char* runId, int result, int time, int mem) {
	...
	try {
		...
		const char* sql = "update status set result=?,time=?,mem=? where run_id=?";
		ps = conn->prepareStatement(sql);
...
void judge(char* runId, char* cid, char* pno, char* lang) {
	...
	try {
		...
		const char* sql = "select tpc_tl,tpc_ml from topiccontent where tpc_id=?";
		sql::PreparedStatement* ps = conn->prepareStatement(sql);
		ps->setString(1,pno);

		sql::ResultSet* rs = ps->executeQuery();
		if (rs->next()) {
			timeLimit = rs->getInt(1); // ms
			memLimit = rs->getInt(2);  // MB
		}
	...
	try {
		...
		const char* sql = "select datain,dataout from datainout where tpc_id=?";
		sql::PreparedStatement* ps = conn->prepareStatement(sql);
		ps->setString(1,pno);
		sql::ResultSet* rs = ps->executeQuery();
